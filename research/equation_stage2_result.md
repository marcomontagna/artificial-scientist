...
Next stage: Claude reviewed the paired local-versus-wide domain challenge, which is now [complete](equation_stage3_result.md). It tests whether a fixed full quadratic approximation can be falsified beyond its training range while accounting for coefficient uncertainty. Active acquisition and grammar expansion remain deferred; the first experiment's failed recovery gate remains a failure.
