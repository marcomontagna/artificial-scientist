...
The first two context selectors have run; see [results](../RESULTS.md). V2 removes parameter-only false expansions in the development sample, but delays useful memory and trails the mixture. Next independently review return-to-simple/changed-lag worlds and a factorial ablation separating faster adaptation from reversible selection. Active experiment choice, raw pixels, richer universes and visual planetary simulations are later stages, conditional on useful evidence.
