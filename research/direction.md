...
The completed [research pass](research_reset.md) provisionally selects detecting wrong causal assumptions through chosen interventions, starting with mathematical and small-world feasibility. That [population check](causal_feasibility_result.md) now passes. The bounded finite-sample diagnostic check is one component study, not the project destination. After it, prioritize a tiny interactive unknown-rule world in which a learner actually chooses informative actions and improves predictions; do not continue an open-ended chain of isolated diagnostic studies. No candidate is novelty-cleared. Multiple universes remain central. New methods need a precise unresolved difference and independent review before implementation; a negative novelty decision is acceptable.
